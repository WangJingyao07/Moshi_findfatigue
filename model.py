__author__ = 'WJY: https://kaggle.com'
import os
import time
import numpy as np
...
DOWNSAMPLE = 20
WIDTH, HEIGHT, NUM_CHANNELS = 4*640 // DOWNSAMPLE, 4*480 // DOWNSAMPLE, 3
NUM_CLASSES = 10
...
class Model:
    def __init__(self, layers, fold_index, batch_size):
	    ...
	    #self.loss_op = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits=self.y_, labels=self.y)
            ...
            tf.summary.scalar('loss_ema', loss_ema.average(self.loss_op))
        ...
        with tf.name_scope("test"):
            ...
            tf.summary.scalar('accuracy_ema', accuracy_ema.average(self.accuracy_op))
        ...
        self.summary_writer = tf.summary.FileWriter(summary_run_path, self.sess.graph)

        tf.train.write_graph(self.sess.graph, MODEL_PATH, 'model.pb', as_text=False)
	...
	num_batches = 0
	    ...
	    accuracy_all += accuracy
            ...
            batch_index += 1
	print loss_all
	print accuracy_all/num_batches
        self.saver.save(self.sess, self.checkpoint_dest, global_step=global_step)
    def train_by_lyt(self, X_train, y_train, epoch):
	...
	X_train, y_train = shuffle(X_train, y_train)
        for batch_start in range(0, num_samples, self.batch_size):
            ...
            _,batch_predictions, batch_loss, batch_accuracy = self.sess.run([
		self.train_op,
                self.y,
                self.loss_op,
                self.accuracy_op
        ...
        score = log_loss(y_train, predictions)
