...
NUM_EPOCHS = 40 if not FLAGS.test else 1
MAX_FOLDS = 3
BATCH_SIZE = 50
...
for train_index, valid_index in LabelShuffleSplit(driver_indices, n_iter=MAX_FOLDS, test_size=0.1, random_state=67):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
	    ...
	    #model.train(X_train, y_train, epoch)
            loss, accuracy, score = model.train_by_lyt(X_train, y_train, epoch)
